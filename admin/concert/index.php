    ...
    <!-- Book show -->
      <div class="row pt-5 mt-3 mb-5">
        <div class="col-sm-12 mb-1">
          <h3 class="display-4 mt-auto" style="width:15%;font-size: 1.6rem;float: left;"><i class="fas fa-calendar text-light fa-1x mr-3 text-dark mr-3"></i><strong class="pt-5">Concert</strong></h3> <a href="" class="btn btn-success" data-toggle="modal" data-target="#eventadd"><i class="fas fa-plus-circle pr-2 icon"></i>Add Concert</a>
        <hr>
        </div>
        ...
        <div class="bg-white border">
          ...
          while($row = mysqli_fetch_assoc($result)){?>
            <tr>
              <td><img src="<?php echo "../../".$row['concert_img'];?>" alt="image" class="img-fuid rounded" width="120"></td>
              <td><?php echo $row['concert_name'];?></td>
              <td><?php echo $row['concert_date'];?></td>
	      <td><?php echo $row['concert_place'];?></td>
              <td>
    ...
    <div class="modal-content">
      ...
      <div class="row">
        <div class="col-sm-6  bg-white pt-2">
            ...
            <input type="text" name="place" value="" placeholder="" class="form-control mb-2" required>
         </div>
         <div class="col-sm-5 mb-2 bg-white pt-3">
           ...
           <input type="submit" name="addconcert" value="Add concert" class="btn btn-primary w-75 mt-5 small-btn">
        </div>
        </div>
       </form>
        </div>
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
        ...
        $send_place = $_POST['place'];
        $send_date = $_POST['date'];
        $send_img = "img/".basename($_FILES["image"]["name"]);
        $sql_event = "insert into concert(concert_img,concert_name,concert_date,concert_place) value('$send_img','$send_name','$send_date','$send_place')";
...
if (isset($_GET['delete'])) {
  ...
  $del = "delete from concert where id='$id'";
